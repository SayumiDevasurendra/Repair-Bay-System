...
export default function ChatbotPopup() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[60] sm:bottom-6 sm:right-6">
      {open && (
        <section className="mb-4 w-[calc(100vw-2rem)] max-w-[380px] rounded-2xl border border-slate-600/40 bg-slate-900/95 shadow-2xl shadow-cyan-900/25 backdrop-blur-lg overflow-hidden">
          ...
          <div className="custom-scrollbar max-h-[360px] overflow-y-auto px-3 py-3 space-y-2 bg-[radial-gradient(circle_at_top_right,rgba(56,189,248,0.10),transparent_42%),linear-gradient(to_bottom,rgba(2,6,23,0.8),rgba(2,6,23,0.98))]">
                ...
                <p
                  className={`max-w-[82%] rounded-2xl px-3 py-2 text-sm leading-relaxed ${
                    message.role === 'user'
                      ...
                      : 'bg-slate-800/85 border border-slate-700 text-slate-100 rounded-tl-sm'
                  }`}
                >
                  {message.content}
                </p>
          ...
          <form onSubmit={sendMessage} className="border-t border-slate-700/70 bg-slate-900/95 p-3">
            <div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800/60 px-2 py-1.5 focus-within:border-cyan-400/60 transition-colors">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about repair-bay safety..."
                className="w-full bg-transparent px-2 text-sm text-slate-100 placeholder:text-slate-500 outline-none"
              />
              <button
                type="submit"
                disabled={!canSend}
                className="inline-flex items-center justify-center rounded-lg bg-cyan-400 px-2.5 py-2 text-slate-950 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-cyan-300 transition-colors"
      ...
      >
        <span className="absolute -inset-1 -z-10 rounded-full bg-cyan-300/30 blur-md group-hover:bg-cyan-300/40 transition-colors" />
